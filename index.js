window.onload = function () {
  //recuerda weon todo va aca en niun otro lado

  //instalacion de context y canvases
  ...
  const castRay = function (srcX, srcY, angle) {
    ...
    while (!isHit && dst < scene.width) {
      ...
      if (rayX > screenW - 4 || rayX < 4 || rayY < 4 || rayY > screenH - 4) {
        isHit = true;
        sceneData.push({ h, val: 5 });
      } else if (
        map[row][col] > 0 &&
        map[row][col] < Object.keys(colorMap).length
      ) {
        isHit = true;
  ...
  document.body.onkeydown = function (e) {
    if (e.keyCode === 37) camera.rotation -= 0.1;
    else if (e.keyCode === 39) camera.rotation += 0.1;
  };
